...
self.addEventListener('fetch', ev => {
  ...
  if (url.pathname.startsWith('/slm/api/')) {
    ev.respondWith(
      fetch(req).catch(() => {
        ...
        return new Response(
          JSON.stringify({ error: 'Network unavailable – cannot fetch live data.' }),
  ...
  if (req.mode === 'navigate' || req.headers.get('accept')?.includes('text/html')) {
    ev.respondWith(
      fetch(req)
        .then(networkRes => {
          // Successful response → store a copy for offline use.
        ...
        .catch(() => {
          // Offline → try a previously‑cached page, otherwise the fallback.
